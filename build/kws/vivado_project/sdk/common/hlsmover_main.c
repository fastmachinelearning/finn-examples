...
int main(int argc, char** argv) {
...
#endif
    XTime_GetTime(&start);
    finn_cnv_w1a1_sw(tx_buffer_ptr, rx_buffer_gld_ptr, CMD_N, INPUT_N_FEATURES, OUTPUT_N_FEATURES);
    XTime_GetTime(&stop);
    ...
    printf("INFO: Total accelerator execution time: %.8f sec\n\r", hw_elapsed);
    printf("INFO: Average accelerator execution time: %.8f sec\n\r", hw_elapsed / CMD_N);
    printf("INFO: Cache flush time: %.8f sec\n\r", cache_elapsed);
    ...
    float error_rate = hw_errors / ((float) OUTPUT_N_FEATURES * CMD_N);
    unsigned correct_classifications = OUTPUT_N_FEATURES * CMD_N - hw_errors;
    float accuracy = correct_classifications / ((float) OUTPUT_N_FEATURES * CMD_N);
    printf("INFO: Total errors = %d (out of %d elements, error rate = %.1f%%)\n\r", hw_errors, OUTPUT_N_FEATURES * CMD_N, error_rate * 100);
    printf("INFO: Correct classifications = %d (out of %d elements, accuracy = %.1f%%)\n\r", OUTPUT_N_FEATURES * CMD_N - hw_errors, OUTPUT_N_FEATURES * CMD_N, accuracy * 100);
    if (hw_errors > 0)
